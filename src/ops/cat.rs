...
impl<S: StorageTrait> TensorBase<S> {
    ...
    pub fn cat<D: Dim>(tensors: &[Self], dim: D) -> Result<Tensor> {
        Self::cat_impl(tensors, dim)
    }

    fn cat_impl<D: Dim>(tensors: &[Self], dim: D) -> Result<Tensor> {
        if tensors.is_empty() {
        ...
        Self::cat_dispatch(tensors, &new_shape, dim_idx, first_dtype)
    }

    /// Main cat implementation dispatcher - similar to softmax_impl
    fn cat_dispatch(
        tensors: &[Self],
        new_shape: &Shape,
        dim_idx: usize,
        ...
        let can_use_contiguous = Self::can_cat_contiguous(tensors, dim_idx);

        macro_rules! dispatch_cat {
            ($ty:ty) => {{
                let out = UninitVec::<$ty>::new(total_elements).init_with(|dst| {
                    if can_use_contiguous {
                        Self::cat_contiguous::<$ty>(tensors, new_shape, dim_idx, dst)
                    } else {
                        Self::cat_general::<$ty>(tensors, new_shape, dim_idx, dst)
                    }
                });
        ...
        match dtype {
            ...
            _ => anyhow::bail!("cat function not supported for dtype: {:?}", dtype),
        }
    }

    /// Check if tensors can use contiguous memory optimizations
    fn can_cat_contiguous(tensors: &[Self], _dim_idx: usize) -> bool {
        if tensors.is_empty() {
            return false;
        }
    ...
    fn cat_contiguous<T: crate::TensorElement>(
        tensors: &[Self],
        new_shape: &Shape,
        dim_idx: usize,
        dst: &mut [T],
        ...
        // Fast path for concatenating along the first dimension (dim_idx == 0)
        if dim_idx == 0 {
            ...
            for tensor in tensors {
                ...
                unsafe {
                    core::ptr::copy_nonoverlapping(src_ptr, dst_ptr, tensor_size);
                }
                dst_offset += tensor_size;
            }
            return;
        }

        // Fast path for concatenating along the last dimension (dim_idx == rank - 1)
        if dim_idx == rank - 1 {
            let outer_size = if rank > 1 {
                (0..rank - 1).map(|i| new_shape[i]).product::<usize>()
            } else {
                1
            };
            let mut dst_offset = 0;
            ...
            for outer_idx in 0..outer_size {
                for tensor in tensors {
                    let inner_size = tensor.shape[dim_idx];
                    let src_offset = outer_idx * inner_size;
                    let src_ptr = tensor.as_ptr() as *const T;
                    let dst_ptr = unsafe { dst.as_mut_ptr().add(dst_offset) };
                    ...
                    dst_offset += inner_size;
                }
            }
            return;
        }

        // Fast path for middle dimensions
        let outer_size = (0..dim_idx).map(|i| new_shape[i]).product::<usize>();
        let inner_size = (dim_idx + 1..rank).map(|i| new_shape[i]).product::<usize>();

        for outer_idx in 0..outer_size {
            let mut concat_offset = 0;
            for tensor in tensors {
                let tensor_dim_size = tensor.shape[dim_idx];
                for dim_idx_val in 0..tensor_dim_size {
                    let src_offset =
                        outer_idx * tensor.shape[dim_idx] * inner_size + dim_idx_val * inner_size;
                    let dst_offset = outer_idx * new_shape[dim_idx] * inner_size
                        + (concat_offset + dim_idx_val) * inner_size;
                ...
                concat_offset += tensor_dim_size;
            }
        }
    }

    /// General cat implementation for non-contiguous tensors
    fn cat_general<T: crate::TensorElement>(
        tensors: &[Self],
        new_shape: &Shape,
        dim_idx: usize,
        dst: &mut [T],
        ...
        let chunk_size = 1024.min(total_elements);
        ...
        for chunk_start in (0..total_elements).step_by(chunk_size) {
            ...
            for pos in chunk_start..chunk_end {
                ...
                // locate source tensor by prefix sums (binary search)
...
mod tests {
    ...
    fn cat_2d_dim0_f32() {
        ...
        let out = Tensor::cat(&[a_src, b_src], 0).unwrap();
