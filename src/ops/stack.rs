...
impl<S: StorageTrait> TensorBase<S> {
    ...
    pub fn stack<D: Dim>(tensors: &[&Self], dim: D) -> Result<Tensor> {
        ...
        macro_rules! dispatch_stack {
            ($ty:ty) => {{
                let out = UninitVec::<$ty>::new(total_elements)
                    .init_with(|dst| Self::_stack_fill::<$ty>(tensors, &new_shape, dim_idx, dst));
                Tensor::from_vec(out, new_shape)
            }};
        }

        // Handle different data types with deduplicated macro
        match first_dtype {
            ...
            _ => anyhow::bail!("stack function not supported for dtype: {:?}", first_dtype),
        }
    }

    /// Fast filler without per-element at(): compute source offset via strides
    fn _stack_fill<T: crate::TensorElement>(
        tensors: &[&Self],
        new_shape: &Shape,
        dim_idx: usize,
        dst: &mut [T],
        ...
        if dim_idx == 0 {
            ...
            let mut all_contig = true;
            // Compute expected row-major strides for inputs
            let mut expected = Shape::empty().with_len(in_rank);
            if in_rank > 0 {
                expected[in_rank - 1] = 1;
                for i in (0..in_rank - 1).rev() {
                    expected[i] = expected[i + 1] * tensors[0].shape[i + 1];
                }
            }
            for t in tensors.iter() {
                // same shape already validated by stack() logic
                for i in 0..in_rank {
                    if (i < t.strides.len() && t.strides[i] != expected[i])
                        || (i >= t.strides.len() && expected[i] != 0)
                    {
                        all_contig = false;
                        break;
                    }
            ...
            if all_contig {
                ...
                for t in tensors.iter() {
                    // Safety: dst has capacity total elements; we copy exactly elem_count_per per tensor
        ...
        // Decode linear index of output, map to source tensor and input indices
