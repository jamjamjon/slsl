...
impl<S: StorageTrait> TensorBase<S> {
    ...
    fn convert_from_typed<Src: TensorElement + Copy, T: TensorElement + Copy>(
        ...
        unsafe {
            if self.is_contiguous() {
                // Fast path: contiguous tensor conversion
                self.convert_contiguous_data::<Src, T>(dst_ptr)?
            } else {
                // Strided conversion for non-contiguous tensors
    ...
    /// Convert contiguous tensor data with optimized memory access
    #[inline(always)]
    unsafe fn convert_contiguous_data<Src: TensorElement + Copy, T: TensorElement + Copy>(
        ...
        // Vectorized conversion using chunks for better performance
        const CHUNK_SIZE: usize = 64; // Process 64 elements at a time
        let chunks = numel / CHUNK_SIZE;
        let remainder = numel % CHUNK_SIZE;

        // Process chunks
        for chunk_idx in 0..chunks {
            let chunk_start = chunk_idx * CHUNK_SIZE;
            for i in 0..CHUNK_SIZE {
                let src_val = *src_ptr.add(chunk_start + i);
                let dst_val = T::from_f32(src_val.to_f32());
                *dst_ptr.add(chunk_start + i) = dst_val;
            }
        }
        ...
        let remainder_start = chunks * CHUNK_SIZE;
        for i in 0..remainder {
            let src_val = *src_ptr.add(remainder_start + i);
            let dst_val = T::from_f32(src_val.to_f32());
            *dst_ptr.add(remainder_start + i) = dst_val;
        }
    ...
    unsafe fn convert_strided_data<Src: TensorElement + Copy, T: TensorElement + Copy>(
        ...
        // Use optimized dimension iterator for strided access
